...
// 1. Define Task Names
const String taskSyncData = "sync_heart_rate_data";

// 2. The Callback Dispatcher (Must be top-level/static)
...
void callbackDispatcher() {
  Workmanager().executeTask((task, inputData) async {
    ...
    try {
      // // Initialize Firebase (Required for background threads)
      ...
      // Return true to indicate the task finished (even if failed), 
      // or false to retry. Usually true avoids infinite retry loops on bad data.
      return Future.value(true); 
    }
  });
}
...
// 3. The Sync Logic
Future<void> _handleSyncTask(StorageService storage) async {
  final syncBox = storage.syncBox;
  
  if (syncBox.isEmpty) {
    print("👷 [Workmanager] No data to sync.");
    return;
  }

  print("👷 [Workmanager] Found ${syncBox.length} items to sync.");

  // Get all data as a Map (Key: Timestamp, Value: JSON Data)
  final rawMap = syncBox.toMap();

  // Send to Firestore in Batch
  final firestoreService = FirestoreService();
  final syncedKeys = await firestoreService.syncRawMaps(rawMap);

  // If successful, delete from local sync queue
  if (syncedKeys.isNotEmpty) {
    await storage.clearSyncedData(syncedKeys);
    print("👷 [Workmanager] Cleared ${syncedKeys.length} items from local queue.");
  }
}

// 4. The Service Class to call from UI
